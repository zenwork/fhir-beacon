...
export function formatHtml(
	...
	options: {
		...
		preserveCommentIndentation?: boolean;
	} = {},
): string {
	const { indentSize = 2, preserveCommentIndentation = true } = options;
	...
	const scriptStyleBlocks: string[] = [];
	html = html.replace(
		/(<(script|style)(?:\s[^>]*)?>)([\s\S]*?)(<\/\2>)/gi,
		(
			_match: string,
			open: string,
			_tag: string,
	...
	// Restore comments
	html = html.replace(
		/<!--__COMMENT_BLOCK_(\d+)__-->/g,
		(_match: string, index: string) => {
			return `<!--${commentBlocks[Number.parseInt(index)]}-->`;
		},
	);

	// Restore script and style blocks
	html = html.replace(
		/<(script|style)(?:\s[^>]*)?>__SCRIPT_STYLE_BLOCK_(\d+)__<\/\1>/gi,
		(_match: string, tag: string, index: string) => {
			return `<${tag}>\n${scriptStyleBlocks[Number.parseInt(index)].trim()}\n</${tag}>`;
		},
	);

	// Restore pre blocks
	html = html.replace(
		/<pre(?:\s[^>]*)?>__PRE_BLOCK_(\d+)__<\/pre>/gi,
		(_match: string, index: string) => {
			return `<pre>${preBlocks[Number.parseInt(index)]}</pre>`;
		},
	);
