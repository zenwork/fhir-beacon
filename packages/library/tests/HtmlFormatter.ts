...
function formatHtml(htmlString, options = {}) {
  ...
  const preBlocks = [];
  html = html.replace(/(<pre(?:\s[^>]*)?>)([\s\S]*?)(<\/pre>)/gi, (match, open, content, close) => {
    preBlocks.push(content);
    return `${open}__PRE_BLOCK_${preBlocks.length - 1}__${close}`;
  });

  // Preserve content in script and style tags
  const scriptStyleBlocks = [];
  html = html.replace(/(<(script|style)(?:\s[^>]*)?>)([\s\S]*?)(<\/\2>)/gi, (match, open, tag, content, close) => {
  ...
  // Restore comments
  html = html.replace(/<!--__COMMENT_BLOCK_(\d+)__-->/g, (match, index) => {
    return `<!--${commentBlocks[index]}-->`;
  });
  
  // Restore script and style blocks
  html = html.replace(/<(script|style)(?:\s[^>]*)?>__SCRIPT_STYLE_BLOCK_(\d+)__<\/\1>/gi, (match, tag, index) => {
    return `<${tag}>\n${scriptStyleBlocks[index].trim()}\n</${tag}>`;
  });
  
  // Restore pre blocks
  html = html.replace(/<pre(?:\s[^>]*)?>__PRE_BLOCK_(\d+)__<\/pre>/gi, (match, index) => {
