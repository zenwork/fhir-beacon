...
import cs_address_type from "./codesystems/cs-address-type.json" with {
	type: "json",
};
import cs_address_use from "./codesystems/cs-address-use.json" with {
	type: "json",
};
import cs_icd_10_procedures from "./codesystems/cs-icd-10-procedures.json" with {
	type: "json",
};
import cs_identifier_use from "./codesystems/cs-identifier-use.json" with {
	type: "json",
};

import cs_observation_status from "./codesystems/cs-observation-status.json";
import cs_quantity_comparator from "./codesystems/cs-quantity-comparator.json" with {
	type: "json",
};
import cs_slotstatus from "./codesystems/cs-slotstatus.json" with {
	type: "json",
};
import cs_v3_orderableDrugForm from "./codesystems/cs-v3-orderableDrugForm.json" with {
	type: "json",
};
import { CodeIds } from "./types";
import vs_age_units from "./valuesets/vs-age-units.json" with { type: "json" };

import vs_data_absent_reason from "./valuesets/vs-data-absent-reason.json" with {
	type: "json",
};
import vs_distance_unit from "./valuesets/vs-distance-units.json" with {
	type: "json",
};
import vs_duration_unit from "./valuesets/vs-duration-units.json" with {
	type: "json",
};
import vs_identifier_type from "./valuesets/vs-identifier-type.json" with {
	type: "json",
};
import vs_identifier_use from "./valuesets/vs-identifier-use.json" with {
	type: "json",
};
import vs_observation_category from "./valuesets/vs-observation-category.json" with {
	type: "json",
};
import vs_observation_interpretation from "./valuesets/vs-observation-interpretation.json" with {
	type: "json",
};
import vs_observation_methods from "./valuesets/vs-observation-methods.json" with {
	type: "json",
};
import vs_observation_status from "./valuesets/vs-observation-status.json" with {
	type: "json",
};
import vs_observation_vitalsignresult from "./valuesets/vs-observation-vitalsignresult.json" with {
	type: "json",
};
import vs_quantity_comparator from "./valuesets/vs-quantity-comparator.json" with {
	type: "json",
};
import vs_referencerange_appliesto from "./valuesets/vs-referencerange-appliesto.json" with {
	type: "json",
};
import vs_ucum_common from "./valuesets/vs-ucum-common.json" with {
	type: "json",
};
...
export class Codes {
	...
	constructor() {
		...
		this.#map.set(
			...
			vs_observation_vitalsignresult,
		);
	...
	getBySystem(url: string): Choices | undefined {
		if (!url || !url.startsWith("http") || url === "http://snomed.info/sct")
			return undefined;
		let lookup = url;
		if (this.#translations.has(url)) lookup = this.#translations.get(url)!;
		const choices: Choices | undefined = Array.from(this.#map.values()).find(
			(set) => set.system === lookup,
		);
	...
	public getAllSystemsAsChoices(): Choices {
		return {
			...
			type: "fhir-beacon",
			choices: Array.from(this.#map.values()).map(
				(v) => ({ value: v.system, display: v.id }) as Choice,
			),
		} as Choices;
	}
}
