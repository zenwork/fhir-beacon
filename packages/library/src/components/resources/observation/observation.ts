...
export class Observation extends DomainResource<ObservationData> {
	...
	public validate(
		...
		validations.inspectCode({
			node: "status",
			code: data.status,
			id: "cs-observation-status",
		});
	...
	public renderAny(
		...
		return [
          ...
          ${oneOf(this, "effective[x]", validations.msgFor("effective[x]"), [
						choice(
          ...
          ${oneOf(this, "value[x]", validations.msgFor("value[x]"), [
						choice(
							data.valueQuantity,
							(d: QuantityData) => html`
                                 <fhir-quantity
