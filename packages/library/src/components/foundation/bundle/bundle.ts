...
} from "../../../shell/layout/renderBackboneCollection";
...
export class Bundle extends BaseElement<BundleData> {
	...
	public renderDisplay(
		...
		return [
          ...
          ${
						hasSome(data.link, this.verbose)
							? renderBackboneCollection(
									"links",
									"link",
									data.link,
        ...
        `,
          ...
          ${
						hasSome(data.entry, this.verbose)
							? renderBackboneCollection(
									"entries",
									"entry",
									data.entry,
...
export function isDomainResource(val: unknown): val is DomainResourceData {
	...
	const type: FhirType | undefined = FhirTypes.find(
		(t) => t.code === (val as { resourceType: string }).resourceType,
	);
	const isResource: boolean = type?.kind === "resource";
	const hasTextProp: boolean =
		typeof val === "object" &&
		true &&
		"text" in val &&
