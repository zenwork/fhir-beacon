...
export function asReadable(
	text: string,
	to: "none" | "lower" | "upper" = "none",
): string {
	if (!text) return text;

	const readable = cleanUpString(
		splitAlphaNumeric(splitAllCapsToCamelCase(splitAllSpecialCase(text))),
	);
...
function splitAllSpecialCase(str: string): string {
	return str.replace(camelCasePattern, "$1 $2").replace(kebabCasePattern, "_");
}
