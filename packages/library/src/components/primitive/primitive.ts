...
import { FhirDataContext, dataContext } from "../../internal/contexts";
...
import { PrimitiveType, convertToPrimitiveType } from "./type-converters";
...
export class Primitive extends ConfigurableElement {
	...
	constructor() {
		super();
		this.validator = new PrimitiveValidator(
			this as unknown as PrimitiveValueHost,
		);
	}
	...
	private renderValid(): TemplateResult {
		...
		if (this.context && this.verbose)
			elements.push(html`
          <fhir-context .text="${this.context ?? ""}${this.context && this.verbose ? " - " : ""}${
						this.verbose ? this.type : ""
					}"
          ></fhir-context>`);

		if (this.mode === DisplayMode.structure)
			elements.push(html`
        <fhir-badge-group ?required=${this.required} ?summary=${this.summary}></fhir-badge-group>`);

		return elements.length > 1 || !isBlank(this.value) || this.verbose
			? html`<li> ${elements}</li>`
			: html``;
	}
	...
	private renderInput(): TemplateResult {
      ...
      `;
		...
		if (
			...
			this.type === PrimitiveType.instant
		) {
			return html`
	...
	private handleChange = (e: Event) => {
		...
		if (
			e.type === "sl-change" &&
			(e.target as SlSwitch).tagName === "SL-SWITCH"
		) {
		...
		this.errormessage = "";
		this.dispatchEvent(
			new PrimitiveInputEvent(this.key, oldValue, this.value, this.type),
		);
	};
