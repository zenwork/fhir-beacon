...
export class PrimitiveValidator {
	...
	validate({
		...
		if (valuePathChanged && this.#host.contextData) {
			if (!isBlank(value) && valuePathChanged) {
				console.warn(
					"primitive: valuePath is overriding value attribute. Do not set both",
				);
			}
		...
		const watchedHaveChanged =
			valueChanged || typeChanged || requiredChanged || choicesChanged;
			...
			if (!isBlank(value) && this.#host.type) {
				...
				choose(this.#host.type, [
					...
					[
						...
						() => this.validOrError(toCanonical, value!),
					],
					...
					[
						...
						() => this.validOrError(toFhirString, value),
					],
					[
						PrimitiveType.forced_error,
						() => {
							this.#host.presentableValue = value;
					...
					[
						...
						() => this.validOrError(toInteger64, value),
					],
					...
					[
						...
						() => this.validOrError(toPositiveInt, value),
					],
					[
						PrimitiveType.string_reference,
						() => this.validOrError(toType, value!),
					],
					[
						PrimitiveType.unsigned_int,
						() => this.validOrError(toUnsignedInt, value),
					],
		...
		if (isBlank(value) && this.#host.required) {
			...
			const event = new PrimitiveInvalidEvent(
				{ path: [{ node: this.#host.key || "unknown" }] },
				value,
				this.#host.type,
		...
		if (errormessageChanged) {
			if (!isBlank(this.#host.errormessage)) {
				this.#host.presentableError =
					this.#host.errormessage ?? "unknown error";
				this.#host.error = true;
				const event = new PrimitiveInvalidEvent(
					{ path: [{ node: this.#host.key || "unknown" }] },
					value,
	...
	private validOrError = <O, V>(fn: (original: O) => V, original: O) => {
		...
		if (!isBlank(parsedValue.val)) {
			...
			this.#host.error = false;
			validEvent = new PrimitiveValidEvent(
				this.#host.key!,
				original,
				this.#host.type,
			);
		...
		if (parsedValue.err) {
			...
			this.#host.error = true;
			invalidEvent = new PrimitiveInvalidEvent(
				{ path: [{ node: this.#host.key! }] },
				original,
				this.#host.type,
