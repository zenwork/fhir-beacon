...
describe("toType", () => {
	...
	it("should throw a TypeError if the value is not a valid FHIR type", () => {
		...
		expect(() => toType(invalidValue)).toThrow(
			`${invalidValue} is not one of the accepted canonical resource types. see: http://hl7.org/fhir/R5/valueset-resource-types.html`,
		);
	});
});
