 ...
 */
export const toDecimal: toPrimitive<unknown, Decimal> = function (
	decimal: unknown,
) {
	if (!decimal)
	...
	if (!regex.test(decimal + ""))
		throw new TypeError(`decimal must be a valid number: ${decimal}`);
	if (isNaN(Number.parseFloat(decimal + "")))
		throw new TypeError(`decimal must be a valid number: ${decimal}`);
	...
	const totalDigits =
		baseDigits.length + (exponentPart !== 0 ? Math.abs(exponentPart) : 0);
...
export function isDecimal(arg: unknown): arg is Decimal {
	if (typeof arg === "string") {
		return (
			Number.isFinite(Number.parseFloat(arg)) && regex.test(arg.toString())
		);
	}
