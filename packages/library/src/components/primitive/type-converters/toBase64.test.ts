...
describe("toBase64 Function", () => {
	...
	it("should throw an error if input is not a string", () => {
		const input = 12345;
		expect(() => toBase64(input as unknown as string)).toThrow(
	...
	it("should throw an error for a base64 string that is too long", () => {
		...
		expect(() => toBase64(inputExample, 1000)).toThrow(
			"Binary size should not exceed 1000",
		);
	});
