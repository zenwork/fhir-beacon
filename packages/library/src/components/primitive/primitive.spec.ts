...
describe("fhir Primitive", () => {
	describe("logic", () => {
      ...
      `).first();
			assert.isEmpty(
				el.queryShadow<PrimitiveLabel>({ select: "fhir-label", expect: 0 }),
			);
		});
      ...
      `).first();
			...
			assert.ok(
				el.queryShadow<HTMLSpanElement>({ select: "fhir-context", expect: 0 }),
			);
		});
      ...
      `).first();
			...
			const span = el.queryShadow<HTMLSpanElement>({
				select: ["fhir-context", "span"],
			});
      ...
      `).first();
			...
			const div = el.queryShadow<HTMLSpanElement>({
      ...
      `).first();
			...
			expect(div.textContent).to.equal(
				"Input must be a non-negative integer within the range 1 to" +
      ...
      `).first();
		...
		test("should display summary flag correctly", async () => {
			const all = await fixture<Primitive>(
				html`
      ...
      `,
			...
			assert.ok(
				all[0].queryShadow<HTMLDivElement>({ select: "fhir-label", expect: 1 }),
			);
			assert.ok(
				all[0].queryShadow<HTMLDivElement>({ select: "fhir-value", expect: 1 }),
			);

			assert.ok(
				all[1].queryShadow<HTMLDivElement>({ select: "fhir-label", expect: 0 }),
			);
			assert.ok(
				all[1].queryShadow<HTMLDivElement>({ select: "fhir-value", expect: 0 }),
			);
		});
	});
	...
	describe("types", () => {
      ...
      `).first();
			...
			const value = el.queryShadow<PrimitiveLabel>({
