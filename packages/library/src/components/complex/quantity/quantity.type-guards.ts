...
import {
	AuthorFhirString,
	AuthorReferenceData,
} from "../annotation/annotation.data";
import { QuantityData, SimpleQuantityData } from "./quantity.data";

export function isQuantity(
	quantity: QuantityData | SimpleQuantityData,
): quantity is QuantityData {
	return (quantity as QuantityData).comparator !== undefined;
}

export function isSimpleQuantity(
	quantity: QuantityData | SimpleQuantityData,
): quantity is SimpleQuantityData {
...
export function isAuthorReference(
	val: ReferenceData | FhirString,
): val is AuthorReferenceData {
	return (
		typeof val !== "string" &&
		!hasNone(val, ["reference", "type", "identifier", "display"])
	);
}

export function isAuthorFhirString(
	val: ReferenceData | FhirString,
): val is AuthorFhirString {
