...
export class Quantity extends BaseElement<QuantityData | SimpleQuantityData> {
	...
	public renderDisplay(
		...
		if (isQuantity(data) && !this.simple) {
            ...
            <fhir-primitive .label=${this.label} .value=${displayValue} .type=${type} summary>
                ${
									data.comparator
										? html`<span slot="before"> ${useSystem(
												"http://hl7.org/fhir/ValueSet/quantity-comparator",
											)
												.choices.find((c) => c.value === data.comparator)
												?.display.toLowerCase()} </span>`
										: nothing
								}
		...
		if (isSimpleQuantity(data) || this.simple) {
            ...
            ${
							validations.msgFor("comparator::sqty-1")
								? html`
	...
	public renderEditableDisplay(
		...
		const comparators: Choices = useSystem(
	...
	public decorate(
		data: QuantityData,
	): Decorated<QuantityData> | Decorated<SimpleQuantityData> {
		// rule: sqty-1
		if (data.comparator) {
			// convert html encoded strings such as &gt;
			data.comparator =
				new DOMParser().parseFromString(data.comparator, "text/html").body
		...
		const isBlankOrUcum =
			!data.system || data.system.toString() === "http://unitsofmeasure.org";

		// rule: dis-1
		const distanceUnits: Choices = useSystem(
			"http://hl7.org/fhir/ValueSet/distance-units",
		);
		if (
			data.unit &&
			distanceUnits.choices.find(
				(d) => data.code === d.value && data.system === distanceUnits.system,
			)
		) {
		...
		// rule: drt-1
		const durations: Choices = useSystem(
		...
		// rule: age-1
		const ageUnits: Choices = useSystem(
			"http://hl7.org/fhir/ValueSet/age-units",
		);
		if (
			data.value &&
			data.value > 0 &&
			ageUnits.choices.find(
				(a) => data.code === a.value && data.system === ageUnits.system,
			)
		) {
