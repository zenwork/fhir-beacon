...
import {
	emptyLitShadow,
	fixture,
} from "../../../../tests/lit/lit-vitest-fixture";
...
describe("Quantity", () => {
    ...
    `).first();
		...
		const primitive = quantity.queryShadow<Primitive>({
			select: "fhir-primitive",
		});
    ...
    `).first();
		...
		expect(primitive).toHaveAttribute("value", "40000");
		expect(primitive.queryShadowNamedSlot("before")[0]).toHaveTextContent(
			"greater than",
		);
    ...
    `).first();
		...
		const elements = quantity.queryShadow<HTMLElement[]>({
			select: "fhir-primitive",
			expect: 6,
		});
		...
		const value = elements[2].queryShadow<HTMLElement>({
			select: "fhir-value",
		});
		const div = value.queryShadow<HTMLDivElement>({ select: "div" });
		expect(div!).toHaveTextContent("abc");
		expect(getComputedStyle(div).textDecoration).toEqual(
			"underline wavy rgb(220, 38, 38)",
		);
