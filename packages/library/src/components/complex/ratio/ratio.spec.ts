...
describe("Ratio", () => {
    ...
    `).first();
		...
		const quantities = ratio.queryShadow<Quantity[]>({
			select: "fhir-quantity",
			expect: 2,
		});
    ...
    `).first();
		...
		const elements = await findAllByShadowText(
			ratio,
			anyMatcher("103.5", "USD", "/", "1", "day"),
			{ depth: 10 },
		);
    ...
    `).first();
		...
		const details = shell.queryShadow<SlDetails[]>({
			select: "sl-details",
			expect: 3,
		});
		...
		// label
		expect(
			details[0].queryShadow<HTMLElement>({ select: "label" }),
		).toHaveTextContent("ratio");

		// id
		const id = shell.queryShadow<Primitive[]>({
			select: "fhir-primitive",
			expect: 13,
		});
		testPrimitive(id[0], { key: "id", value: null });

		const quantities = shell.queryShadow<Quantity[]>({
			select: "fhir-quantity",
			expect: 2,
		});
