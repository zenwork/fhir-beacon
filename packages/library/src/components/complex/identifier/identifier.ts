...
import { BaseElement, Decorated, Validations, errors } from "../../../internal";
...
export class Identifier extends BaseElement<IdentifierData> {
	...
	public validate(
		...
		if (data.use) {
			if (
				!useSystem("http://hl7.org/fhir/identifier-use").choices.find(
					(c) => c.value === data.use,
				)
			) {
				validations.add({
					fqk: { path: [{ node: "use" }] },
					message:
						"identifier use is not one of accepted: " +
						useSystem("http://hl7.org/fhir/address-use")
		...
		if (data.type?.coding) {
			const values = useSystem(
				"http://hl7.org/fhir/ValueSet/identifier-type",
			).choices;
			const codings: CodingData[] = data.type.coding;
			codings
				.filter((coding) => !values.find((v) => v.value === coding.code))
				.forEach((_, index) => {
					validations.add({
						fqk: {
						...
						message:
							"identifier type is not one of accepted: " +
