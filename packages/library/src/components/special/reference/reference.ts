          ...
          ${choose(this.referenceType, [
                  ...
                  `,
                      ...
                      <fhir-primitive
                              ...
                              .value=${data.display}
                              .link=${data.reference}
                              .errormessage=${referenceErrorMessage(validation)}
                              summary
                      ></fhir-primitive >`,
						],
						[
	...
	public renderEditableDisplay(
		...
		return [
          ...
          <fhir-primitive key="reference"
                          .value=${data.reference}
                          .errormessage=${referenceErrorMessage(validations)}
                          summary
          ></fhir-primitive>
		...
		if (this.data && this.data.reference) {
			...
			if (this.containedData && this.containedData.length > 0) {
				...
				if (isContainedRef && containedDataExists) {
					this.mappedResource = this.containedData.find(
						(r) => "#" + r.id === this.data.reference,
					);
				...
				if (
					...
					validations.add({
						fqk: { path: [{ node: "*" }] },
						message:
							"ref-1: Does not have a contained resource when reference starts with #",
					});
					this.referenceType = ReferenceType.unknown;
				}
			}
...
function referenceErrorMessage(validations: Validations): string | undefined {
	return validations.msgFor("*") ?? validations.msgFor("reference");
}
