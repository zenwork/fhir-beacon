...
export function removeValues(
	included: ResolvedValue[],
	excluded: ResolvedValue[],
): ResolvedValue[] {
	const reducedSet: ResolvedValue[] = included.filter(
...
export class ValueSetProcessor {
	...
	process(id: string, debug?: boolean): Promise<Choices[]> {
		return this.#ready.then(() =>
			this.#source.resolve(id, debug).then((resolvedSets: ResolvedSet[]) => {
				return resolvedSets.map((set) => {
					...
					const reducedSet: ResolvedValue[] = removeValues(
						set.compose.include.concept,
						set.compose.exclude.concept,
					);

					const choices: Choice[] = reducedSet.map((c) => ({
						value: c.code,
						display: c.display,
					}));
