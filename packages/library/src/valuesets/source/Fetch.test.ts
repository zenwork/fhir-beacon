...
describe("fetchWithRetry", () => {
	...
	it("should retry the specified number of times if fetch fails", async () => {
		...
		mockFetch.mockRejectedValue(
			new FetchError("Network error", "", -1, "", "", undefined, true),
		);

		await expect(
			fetchIt({ url: url, options: {}, retries: 3, retryFactor: 50 }),
	...
	it("should throw FetchError for non-OK response and then stop retries", async () => {
		...
		await expect(
			fetchIt({ url: url, options: {}, retries: 5, retryFactor: 50 }),
	...
	it("should throw FetchError when response JSON parsing fails", async () => {
		...
		global.fetch = mockFetch;
		mockFetch.mockResolvedValue(
			createResponse(200, url, "Invalid JSON String"),
		);
	...
	it("should return data as json", async () => {
		...
		global.fetch = mockFetch;
		mockFetch.mockResolvedValue(
			createResponse(200, "https://example.com/api", {
