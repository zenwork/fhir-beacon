	...
	function doIt(
		...
		try {
			if (isValueSet(resource))
				return resolveValueSet(resource, skipUrl, debug);
			...
			throw new FetchError(
				`processing bundle entry is not possible`,
				resource.id ?? "n/a",
				0,
				"OK",
	...
	for (const input of inputs) {
		try {
			...
			if (batch.length >= concurrent) {
				const r: O[] = (await Promise.all(batch)).map((each: unknown) =>
					postBatch(each, debug),
				);
				results.push(...r);
				batch = [];
			}
