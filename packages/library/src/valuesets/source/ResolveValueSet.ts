...
export async function resolveValueSet(
	...
	if (isBlank(vs.name)) {
		throw new Error(
			"ValueSet name is required for code generation" +
	...
	if (vs.url?.match(/[|#]/) !== null) {
		throw new Error(
			'ValueSet url must not contain "|" or "#" characters' +
	...
	if (vs.status?.match(/draft | active | retired | unknown/) !== null) {
		throw new Error(
			'ValueSet status must be one of "draft | active | retired | unknown" for code generation' +
...
export async function resolveIncludesOrExclude(
	id: string,
	...
	for (let idx = 0; idx < segment.length; idx++) {
		promises.push(
			resolve(id, segment[idx], idx, debug, skipUrl, urlsToResolve, variant),
		);
	}
	...
	const timeout = (millis: number) => {
		return new Promise<ResolvedValue[][]>((_, rej) => {
			...
			return setTimeout(
				() =>
					rej(`Failed to resolve value set in ${maxTime / 1000}s for ${urls}`),
				millis,
			);
		});
	};
...
function assertOk(inc: ValueSetIncludeExcludeData, idx: number): void {
	if (isBlank(inc.system) && isBlank(inc.valueSet)) {
		throw new Error(
			`ValueSet include system or concept is required for code generation [${idx}] (vsd-1 -> http://hl7.org/fhir/valueset-definitions.html#ValueSet.compose.include)'}`,
		);
	}

	if (inc.concept && inc.concept.length > 0 && isBlank(inc.system)) {
		throw new Error(
			`include concept list requires a system [${idx}] (vsd-2 -> http://hl7.org/fhir/valueset-definitions.html#ValueSet.compose.include)`,
		);
	}

	if (
		inc.concept &&
		inc.concept.length > 0 &&
		inc.filter &&
		inc.filter.length > 0
	) {
		throw new Error(
			`include can only have concept list OR a filter list [${idx}] (vsd-3 -> http://hl7.org/fhir/valueset-definitions.html#ValueSet.compose.include)`,
		);
	}
}
...
function resolve(
	...
	return new Promise<ResolvedValue[]>((resolve, reject) => {
		...
		if (segment.concept && segment.concept.length > 0) {
			...
			Promise.all(
				segment.valueSet.map((vs) =>
					resolveChildValueSet(id, vs, skipUrl, debug, urlsToResolve),
				),
			)
				.then((all: ResolvedSet[][]) => all.filter((v) => v !== null))
				.then((sets) => sets.flat())
				.then((valid: ResolvedSet[]) =>
					valid.map((v) => {
...
function resolveConcepts(
	...
	return Promise.resolve(
		inc.map((c) => ({
			code: c.code,
			display: c.display ?? "n/a",
			definition: "n/a",
		})),
	);
}

function resolveChildSystem(
	id: string,
	system: URI,
	urlsToResolve: { uri: string; resolved: boolean }[],
	skipUrl: (url: string) => boolean,
		...
		return fetchIt({ url: url, debug })
			.then((json: ValueSetData | CodeSystemData | unknown) => {
				if (isCodeSystem(json)) {
					if (debug)
						console.log(
							`resolved [${json.concept.length}] from : ${url} - for : ${id}`,
						);
					concepts.push(
						...json.concept.map((c: Record<string, unknown>) => ({
							code: c.code as string,
							display: (c.display as string) ?? "n/a",
							definition: (c.definition as string) ?? "n/a",
						})),
					);
					urlsToResolve.forEach((u) => {
						if (u.uri === url) u.resolved = true;
					});
				}
				return concepts;
		...
		return fetchIt({ url: httpsUri, debug })
			.then((json: ValueSetData | CodeSystemData | unknown) => {
				if (isValueSet(json)) {
					...
					throw Error(
						`Unknown ValueSet type: ${(json as { resourceType: string }).resourceType}`,
					);
				}
			})
			.then((resolved) => {
				urlsToResolve.forEach((u) => {
					if (u.uri === httpsUri) u.resolved = true;
				});
				return resolved;
			});
	}
...
function convert(uri: string): string {
	...
	if (uri.startsWith("https://hl7.org/fhir/")) {
		return (
			uri.replace("https://hl7.org/fhir/", "https://hl7.org/fhir/codesystem-") +
			".json"
		);
	}
	return uri;
