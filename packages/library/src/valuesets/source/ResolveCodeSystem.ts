...
import {
	CodeSystemConceptData,
	CodeSystemData,
	ResolvedSet,
	ResolvedValue,
} from "../ValueSet.data";

export async function resolveCodeSystem(
	vs: CodeSystemData,
	debug: boolean = false,
): Promise<ResolvedSet[]> {
	if (isBlank(vs.name)) {
		throw new Error(
			"CodeSystem name is required for code generation" +
	...
	if (vs.url?.match(/[|#]/) !== null) {
		throw new Error(
			'CodeSystem url must not contain "|" or "#" characters' +
	...
	if (vs.status?.match(/draft | active | retired | unknown/) !== null) {
		throw new Error(
			'CodeSystem status must be one of "draft | active | retired | unknown" for code generation' +
	...
	return Promise.all([
		Promise.resolve(
			resolveIncludesOrExclude(vs.id ?? "n/a", vs.concept ?? [], debug),
		),
...
function resolveIncludesOrExclude(
	id: string,
	segment: CodeSystemConceptData[],
	debug: boolean,
): ResolvedValue[] {
	if (debug) console.log("resolved [" + segment.length + `] from ${id}`);
	return segment.map((inc) => ({
		code: inc.code,
		display: inc.display ?? "n/a",
		definition: "n/a",
	}));
}
