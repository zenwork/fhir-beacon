...
export class FSSource implements ValueSetSource, LoadableStore {
	...
	constructor(
		...
		public skipUrl: (url: string) => boolean,
	) {
	...
	loadDir(): Promise<boolean> {
		...
		try {
			readdir(this.#basePath).then((files) => {
	...
	loadfile(): Promise<boolean> {
		...
		try {
			lstat(this.#path).then((stats: Stats) => {
				if (stats.isFile()) {
					console.log(
						`loading valuesets: ${this.#loaded} - ${this.#files.length} - ${this.#path}`,
					);
					this.#loaded = true;
			...
			try {
				...
				return realpath(fullpath)
					...
					.then((json) => {
						...
						if (isValueSet(json))
							return resolveValueSet(json, this.skipUrl, debug);
	...
	public cacheAll(debug: boolean = false): Promise<boolean> {
		return Promise.all<boolean>(
			this.#files.map((f) => {
				return this.resolve(f, debug).then((sets: ResolvedSet[]) => {
