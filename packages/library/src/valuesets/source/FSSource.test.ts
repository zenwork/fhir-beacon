...
describe.runIf(process.env.EXPENSIVE)("FSSource", () => {
	...
	test(
		"should read a file and resolve referenced coding system concepts over the internet",
		{ timeout: 30000 },
		async () => {
			const source: FSSource = new FSSource(
				exampleData,
				undefined,
				() => false,
			);
			...
			const valueSet: ResolvedSet = (
				await source.resolve("valueset-transport-intent.json", false)
			)[0];
	...
	test(
		...
		async () => {
			...
			const valueSet: ResolvedSet = (
				await source.resolve("valueset-week-of-month.json", false)
			)[0];
	...
	test(
		"should read a file and resolve referenced value set concepts over the internet",
		{ timeout: 30000 },
		async () => {
			const source: FSSource = new FSSource(
				exampleData,
			...
			const valueSet: ResolvedSet = (
				await source.resolve("valueset-definition-topic.json")
			)[0];
	...
	test(
		"should fail when url are not resolvable",
		{ timeout: 30000 },
		async () => {
			const source: FSSource = new FSSource(
				exampleData,
	...
	test("should fail when file not found", { timeout: 30000 }, async () => {
		const source: FSSource = new FSSource(exampleData, undefined, () => false);
		await source.loadDir();
