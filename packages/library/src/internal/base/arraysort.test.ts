...
describe("array sort", () => {
	test.each([
		...
		},
		{
			input: [
				[
					{ path: [{ node: "a" }, { node: "a", index: 1 }], key: "foo" },
					["foo"],
				],
				[{ path: [{ node: "a" }, { node: "a" }], key: "bar" }, ["foo"]],
			],
		...
		},
		{
			input: [
				[
					{ path: [{ node: "a", index: 5 }, { node: "a" }], key: "bar" },
					["a5abar"],
				],
				[
					{
						path: [{ node: "c", index: 5 }, { node: "a" }],
						key: "bar",
						index: 10,
					},
					["c5abar10"],
				...
				[
					{ path: [{ node: "c", index: 5 }, { node: "a" }], key: "bar" },
					["c5abar"],
				],
				[
					{ path: [{ node: "a", index: 3 }, { node: "a" }], key: "bar" },
					["a3abar"],
				],
			],
			expected: [
		...
		const sorted: [FullyQualifiedKey, string[]][] = sort(
			input as [FullyQualifiedKey, string[]][],
		);
	...
	test.each([
		...
		const reindexed: [FullyQualifiedKey, string[]][] = reindex(
			input as [FullyQualifiedKey, string[]][],
		);
	...
	test.each([
		...
		const reindexed: [FullyQualifiedKey, string[]][] = reindex(
			sort(input as [FullyQualifiedKey, string[]][]),
		);
		// console.log(reindexed.map(e => JSON.stringify(e[0]) + ' : [' + e[1].join(', ') + ']') )
		expect(reindexed).toStrictEqual(expected);
	});
