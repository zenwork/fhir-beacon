...
export class ValidationsImpl<D extends FhirElementData> implements Validations {
	...
	constructor(decorated: Decorated<D>) {
		this.#data = decorated;
	}
	...
	public sliceForFQK(sliceKey: FullyQualifiedKey): FqkMap {
		...
		try {
			...
			const entries: [FullyQualifiedKey, string[]][] = matchingKeys.map(
				(fqk) => {
	...
	public rmAll(): boolean {
		this.#data[errors].keys().forEach((key) => this.#data[errors].delete(key));
		return true;
	}
	...
	public inspectCode({
		...
		code,
		id,
	}: { node: KeyBase } & CodeIdPair): boolean {
		const codes = this.code(id);
		if (codes) {
			const valid: boolean = codes.choices.some(
				(choice) => code === choice.value,
			);
			if (!valid) {
	...
	public inspectCodeableConcept({
		node,
		concept,
		bindingId,
	}: { node: KeyBase } & CodeableConceptIdPair): boolean {
		...
		const validateOne = (coding: CodingData, i: number) => {
			...
			const validCode: boolean = codes.choices.some(
				(choice) => code === choice.value,
			);
			...
			if (!validCode) {
				...
				this.add({
					fqk: {
						path: [err, { node: "coding" }, { node: "code" }],
						key: "binding",
					},
			...
			if (!validSystem) {
				...
				this.add({
					fqk: {
						path: [err, { node: "coding" }, { node: "system" }],
						key: "binding",
					},
	...
	public all(): KeyErrorPair[] {
		return this.#data[errors]
			.entries()
			.flatMap(([fqk, messages]) =>
				messages.map((message) => ({ fqk, message })),
			);
	}
...
export function slice(fqk: FullyQualifiedKey): FullyQualifiedKey {
	...
	copy.path =
		copy.path && copy.path.length > 0 ? copy.path.slice(1) : copy.path;
	return copy;
}

export function matches(
	sliceKey: FullyQualifiedKey,
	storeKey: FullyQualifiedKey,
): boolean {
	const pathMatch: boolean =
		...
		sliceKey.path.every((p, index) => {
			...
			const indexMatch: boolean =
				p.index !== undefined &&
				((storeNode.index === undefined && p.index === 0) ||
					(storeNode.index !== undefined && storeNode.index === p.index));
	...
	const indexMatch: boolean =
		sliceKey.index !== undefined && sliceKey.index === storeKey.index;
...
function isCodeableConcept(
	value: CodeableConceptData | CodeableConceptData[],
): value is CodeableConceptData {
	return !!value && !Array.isArray(value) && "coding" in value;
}

function isCodeableConceptArray(
	value: CodeableConceptData | CodeableConceptData[],
): value is CodeableConceptData[] {
