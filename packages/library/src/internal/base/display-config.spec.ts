...
describe("DisplayConfig", () => {
	describe("primitive", () => {
      ...
      `).first();
			...
			const element = primitive.queryShadowByText(
				"decimal must be a valid number: abc",
			);
			...
			primitive.queryShadow<PrimitiveError>({
				select: ["fhir-error"],
				expect: 0,
			});
		});
      ...
      `).all();
			...
			assert.ok(
				primitives[0].queryShadow({ select: "fhir-context", expect: 0 }),
			);
			expect(
				primitives[1].queryShadowByText("(week - positiveInt)"),
			).toBeVisible();
	...
	describe("complex types", () => {
		it("should show nothing when there is no data and verbose is enabled", async () => {
			const annotation = await fixture<Annotation>(
				html`
        <fhir-shell verbose showerror>
            <fhir-annotation ></fhir-annotation >
        </fhir-shell >
      `,
				new Annotation().tagName,
			).first();
			...
			expect(
				annotation.queryShadowByText(
					"the data or data-path property must be provided",
				),
			).toBeVisible;
		});
      ...
      `,
			...
			assert.ok(
				annotation.queryShadow({ select: "fhir-primitive", expect: 7 }),
			);
			...
			assert.ok(annotation.queryShadow({ select: "fhir-wrapper", expect: 2 }));
			assert.ok(
				annotation.queryShadow({ select: "fhir-primitive", expect: 3 }),
			);
			...
			assert.ok(
				annotation.queryShadow({ select: "fhir-primitive", expect: 0 }),
			);
		});
      ...
      `,
			...
			await aTimeout();
			assert.ok(
				annotation.queryShadow({ select: "fhir-structure-wrapper", expect: 0 }),
			);
			assert.ok(
				annotation.queryShadow({ select: "fhir-primitive", expect: 3 }),
			);
      ...
      `,
			...
			assert.ok(
				annotation.queryShadow({ select: "fhir-primitive", expect: 25 }),
			);
		});
	});
	describe("resource types", () => {
		it("should show resource errors when verbose is enabled", async () => {
			const medication = await fixture<Medication>(
				html`
        <fhir-shell verbose showerror>
          <fhir-medication ></fhir-medication >
        </fhir-shell >
      `,
				new Medication().tagName,
			).first();
