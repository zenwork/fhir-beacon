...
export function reindex(
	original: [FullyQualifiedKey, string[]][],
): [FullyQualifiedKey, string[]][] {
	const copy: [FullyQualifiedKey, string[]][] = JSON.parse(
...
function modify(
	entry: FullyQualifiedKey,
	lookups: { [key: string]: { next: number; previous: number } },
): FullyQualifiedKey {
	const reindexedPaths: ErrorNodeKey[] = [];
	const path: string = entry.path.reduce((prev, p) => {
		const key: string = `${prev}.${p.node}${p.index !== undefined ? `[x]` : ""}`;
		if (!lookups[key]) {
			lookups[key] = { next: 0, previous: p.index ?? 0 };
		}

		if (p.index !== undefined && lookups[key].previous !== p.index)
			lookups[key].next++;
	...
	const key = `${path}.${entry.key !== undefined ? entry.key : ""}${
		entry.index !== undefined ? "[x]" : ""
	}`;
	...
	if (entry.index !== undefined && lookups[key].previous !== entry.index)
		lookups[key].next++;
