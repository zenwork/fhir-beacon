...
describe("ValidationsImpl", () => {
	...
	describe("message for", () => {
		...
		it("should return the error message associated with the given full key", () => {
			const key: FullyQualifiedKey = {
				path: [{ node: "parent" }, { node: "child" }],
				key: "testKey",
			};
			const errorMessage = "Test error message";
	...
	describe("slicing", () => {
		it("should slice map for fqk", () => {
			val.add({
				fqk: {
					path: [{ node: "to" }, { node: "testKey", index: 0 }],
					key: "err",
				},
				message: "A",
			});
			val.add({
				fqk: {
					path: [{ node: "to" }, { node: "testKey", index: 1 }],
					key: "err",
					index: 0,
				},
				message: "B1",
			...
			val.add({
				fqk: { path: [{ node: "to" }, { node: "otherTestKey" }], key: "err" },
				message: "C",
			});

			let key: FullyQualifiedKey = {
				path: [{ node: "to" }, { node: "testKey", index: 0 }],
				key: "err",
			};
			...
			expect(subMap.get({ path: [{ node: "testKey" }], key: "err" })).toEqual([
				"A",
			]);
			...
			expect(
				subMap.get({ path: [{ node: "testKey" }], key: "err", index: 0 }),
			).toEqual(["B1"]);
			expect(
				subMap.get({ path: [{ node: "testKey" }], key: "err", index: 1 }),
			).toEqual(["B2"]);
			...
			expect(
				subMap.get({ path: [{ node: "testKey", index: 0 }], key: "err" }),
			).toEqual(["A"]);
			expect(
				subMap.get({
					path: [{ node: "testKey", index: 1 }],
					key: "err",
					index: 0,
				}),
			).toEqual(["B1"]);
			...
			expect(subMap.keys()).toHaveLength(1);
			expect(
				subMap.get({ path: [{ node: "otherTestKey" }], key: "err" }),
			).toEqual(["C"]);
		});
	});
	...
	describe("codeable concept", () => {
		it("should validate a codeable concept", () => {
			...
			expect(all[0].message).toEqual(
				"code '123000' not in: cs-icd-10-procedures. Valid: 123001, 123002, 123003",
			);
		});
	});
});
