 ...
 */
export abstract class FhirDataElement<T extends FhirElementData>
	...
	implements DataHandling<T>
{
	...
	public abstract isPrepared(
		providedData: T,
		decoratedData: Decorated<T>,
	): void;
	...
	protected willUpdate(changes: PropertyValues): void {
		...
		if (
			(changes.has("data") || changes.has("profile")) &&
			this.shouldPrepare()
		) {
			...
			if (this.extendedData[profile]) {
				this.extendedData[profile].validate(
					this.extendedData,
					validations,
					this.#fetched,
				);
