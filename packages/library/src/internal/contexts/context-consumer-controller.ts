 ...
 */
export class DataContextConsumerController<T extends FhirElementData>
	implements ReactiveController
{
 ...
 */
export class DisplayContextConsumerController<T extends FhirElementData>
	implements ReactiveController
{
		...
		new ContextConsumer(host, {
			...
			callback: (config: DisplayConfig) => {
				if (config) {
					...
					if (config.summaryonly !== undefined)
						host.summaryonly = config.summaryonly;
