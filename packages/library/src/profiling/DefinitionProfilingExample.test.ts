...
describe("profileDefinition", () => {
	it("should be tested", () => {
		const observation = profile<ObservationData>({
			...
			props: [
				define
					.optionalListOf<ObservationData>("identifier", Identifier)
					.isSummary(),
				...
				define
					.optionalListOf<ObservationData>("category", CodeableConcept)
					.boundBy("vs-observation-category", Preferred),

				define
					.oneOf<ObservationData>("code", CodeableConcept)
					.boundBy("vs-observation-codes")
					.isSummary(),

				define
					.optionOf<ObservationData>("subject", Reference, [
						Patient,
						Group,
						Device,
						Location,
				...
				define
					.optionOf<ObservationData>("encounter", Reference, [Encounter])
					.isSummary(),

				define
					.choiceOf<ObservationData>("effective", "DateTime", dateTime)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("effective", "Period", Period)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("effective", "Timing", Timing)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("effective", "Instant", instant)
					.optional()
					.isSummary(),
				...
				define
					.optionalListOf<ObservationData>("performer", Reference, [
						Practitioner,
						PractitionerRole,
						Organization,
						CareTeam,
				...
				define
					.choiceOf<ObservationData>("value", "Quantity", Quantity)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>(
						"value",
						"CodeableConcept",
						CodeableConcept,
					)
					...
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "String", string)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "Boolean", boolean)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "Integer", integer)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "Range", Range)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "Ratio", Ratio)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "SampledData", SampledData)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "Time", time)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "DateTime", dateTime)
					.optional()
					.isSummary(),
				define
					.choiceOf<ObservationData>("value", "Period", Period)
					.optional()
					.isSummary(),

				define
					.optionOf<ObservationData>("dataAbsentReason", CodeableConcept)
					.boundBy("vs-data-absent-reason", Extensible),
				define
					.optionalListOf<ObservationData>("interpretation", CodeableConcept)
					.boundBy("vs-observation-interpretation", Extensible),
				define.optionalListOf<ObservationData>("note", Annotation).optional(),
				define
					.optionOf<ObservationData>("bodySite", CodeableConcept)
					.boundBy("vs-body-site", Example),
				define
					.optionOf<ObservationData>("method", CodeableConcept)
					.boundBy("vs-observation-methods", Example),
				define.optionOf<ObservationData>("specimen", Reference, [Specimen]),
				define.optionOf<ObservationData>("device", Reference, [
					Device,
					DeviceMetric,
				]),

				define
					.backboneListOf<ObservationData>(
						"referenceRange",
						profile<ObservationData>({
							...
							props: [
								...
								define.optionalListOf<ObservationData>(
									"appliesTo",
									CodeableConcept,
								),
				...
				define
					.optionalListOf<ObservationData>("hasMember", Reference, [
						Observation,
						QuestionnaireResponse,
						MolecularSequence,
					])
				...
				define
					.optionalListOf<ObservationData>("derivedForm", Reference, [
						DocumentReference,
						ImagingStudy,
						Media,
						QuestionnaireResponse,
					...
					.backboneListOf<ObservationData>(
						...
						profile<ObservationData>({
							...
							props: [
								...
								define
									.optionOf<ObservationData>(
										"dataAbsentReason",
										CodeableConcept,
									)
									.boundBy("vs-data-absent-reason", Extensible),
								define
									.optionalListOf<ObservationData>(
										"interpretation",
										CodeableConcept,
									)
									.boundBy("vs-observation-interpretation", Extensible),
								...
								define
									.backboneListOf<ObservationData>(
										...
										profile<ObservationData>({
											...
											props: [
												define.optionOf<ObservationData>("low", SimpleQuantity),
												define.optionOf<ObservationData>(
													"high",
													SimpleQuantity,
												),
												define.optionOf<ObservationData>(
		...
		const _bp: StructureDefinition<ObservationData> = profile<ObservationData>({
				...
				slice.oneFor(
					...
					[
						(data: ObservationData, fixedValue: string) => ({
							success:
								data.code.coding?.filter((c) => c.code === fixedValue)
									.length === 1,
							message: "bp: code.coding[0].code must be " + fixedValue,
						}),
						(data: ObservationData, fixedValue: string) => ({
							success:
								data.code.coding?.filter((c) => c.system === fixedValue)
									.length === 1,
							message: "bp: code.coding[0].system must be " + fixedValue,
						}),
					],
					["85354-9", "http://loinc.org"],
				),
				slice.oneFor(
					// name:SystolicBP
					["component"],
						...
						(data: ObservationData, fixedValue: string) => ({
							success:
								data.component?.filter(
									(c) => c.code.coding[0].code === fixedValue,
								).length === 1,
							message:
								"bp: component[0].code.coding[0].code must be " + fixedValue,
						}),
						(data: ObservationData, fixedValue: string) => ({
							success: data.component![0].code.coding[0].system === fixedValue,
							message:
								"bp: component[0].code.coding[0].system must be " + fixedValue,
						}),
					],
				...
				slice.oneFor(
					...
					[
						(data: ObservationData, fixedValue: string) => ({
							...
							message:
								"bp: component[1].code.coding[0].code must be " + fixedValue,
						}),
						(data: ObservationData, fixedValue: string) => ({
							success: data.component![1].code.coding[0].system === fixedValue,
							message:
								"bp: component[1].code.coding[0].system must be" + fixedValue,
						}),
					],
					...
					[
						(data: ObservationData, fixedValue: string) => ({
							success:
								...
								data.valueQuantity?.system === fixedValue,
