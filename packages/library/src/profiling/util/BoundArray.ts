export class BoundArray {
	...
	constructor(
		...
		if (max < min) {
			throw new Error(
				"Max cardinality cannot be smaller than min cardinality.",
			);
		}
	...
	add(element: any) {
		if (this.array.length >= this.max) {
			throw new Error(
				`Cannot add more elements. Max cardinality of ${this.max} reached.`,
			);
		}
	...
	remove(element: any) {
		if (this.array.length <= this.min) {
			throw new Error(
				`Cannot remove the element. Min cardinality of ${this.min} would be violated.`,
			);
		}
	...
	insert(index: number, element: number) {
		if (this.array.length >= this.max) {
			throw new Error(
				`Cannot insert more elements. Max cardinality of ${this.max} reached.`,
			);
		}
