...
import {
	...
	flattenKey,
	isDefWithChildren,
	isDefWithConstraints,
} from "../index";

export function sliceBuilder<T extends Decorateable>(
	def: PropertySliceDef<T>,
): Builder<T> {
		...
		build: () => {
			if (context) {
						...
						(prev, key, index) => {
							if (index == 0 || isDefWithChildren(prev)) {
								...
								if (index === 0) {
									value =
										prev && prev.subdefs
											? prev.subdefs.get(flattenKey(key, def.choice)) || null
											: context!.def.get(key, def.choice);
									if (!value) throw new Error(`No property ${key} found`);
								} else {
									value =
										prev && prev.subdefs
											? prev.subdefs.get(flattenKey(key)) || null
											: context!.def.get(key);
								}
				...
				if (ref && isDefWithConstraints(ref) && isDefWithConstraints(def)) {
					...
					throw new Error(
						`slice must be set on existing property. None found for: ${def.choice}${def.key}`,
					);
				}
			}
		},
