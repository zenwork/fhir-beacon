...
export function definitionBuilder<T extends Decorateable>(
	def: PropertyDef<T>,
): PropertyBuilder<T> {
	...
	const action = {
		...
		build: () => {
			...
			if (existing && context && isPropertyDef(existing)) {
				context.def.set({
					...
					bindings:
						(def.bindings &&
							Array.isArray(def.bindings) &&
							def.bindings.length > 0) ||
						def.bindings
							? def.bindings
							: existing.bindings,
					bindingStrength: def.bindingStrength
						? def.bindingStrength
						: existing.bindingStrength,
					constraints:
						def.constraints.length > 0 ? def.constraints : existing.constraints,
					...
					typeNarrowing: def.typeNarrowing
						? def.typeNarrowing
						: existing.typeNarrowing,
		...
		boundBy: (
			binding: CodeIds | Choice[] | Choices,
