...
export type TypedConstraintAssertion<T> = DefConstraintAssertion<T> & {
	_constraintType?: string;
	_fixedValue?: unknown;
};

export function sliceProp<T>(
	key: string | string[],
	...
	constraints.forEach((c, i) => {
		const constraint: TypedConstraintAssertion<T> =
			c as TypedConstraintAssertion<T>;
