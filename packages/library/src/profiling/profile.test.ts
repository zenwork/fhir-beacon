...
describe("test", () => {
	it("test", () => {
		const base: StructureDefinition<DomainResourceData> = profile({
			type: Observation,
			props: [
				define
					.oneOf<DomainResourceData>("foo", HumanName)
					.optional()
					.boundBy([
						{ value: "baz", display: "baz" },
						{ value: "biff", display: "biff" },
					]),
				define.listOf<DomainResourceData>("baz", Address).optional(),
				define.oneOf<DomainResourceData>("baz", CodeableConcept),
				define
					.backboneOf<DomainResourceData>(
						"backboneKey",
						profile<DomainResourceData>({
							type: new ResourceDef("ObservationFoo"),
							props: [
								define
									.oneOf<DomainResourceData>("bkStuff", CodeableConcept)
									.optional()
									.isSummary(),
		...
		const _aProfile: StructureDefinition<DomainResourceData> = profile({
			...
			props: [
				...
				define.backboneOf<DomainResourceData>(
					"backboneKey",
					profile<DomainResourceData>({
						type: new ResourceDef("ObservationFoo"),
						props: [
