...
export class StructureDefinition<T extends Decorateable> {
	...
	extendRender: Map<DisplayMode, TemplateGenerator<T>> = new Map<
		DisplayMode,
		TemplateGenerator<T>
	>();
	overrideRender: Map<DisplayMode, TemplateGenerator<T>> = new Map<
		DisplayMode,
		TemplateGenerator<T>
	>();
			...
			.map((v: Defs<T>) => {
				if (isPropertyDef(v)) {
					...
					const k: string = (
						indent +
						marker +
						(Array.isArray(v.key) ? v.key.join(".") : v.key)
					).padEnd(50, " ");
					// summary
					...
					i =
						i +
						(v.constraints.some(
							(c) => c._constraintType === "profile-constraint",
						)
							? "π"
							: "");
					// @ts-ignore
					i =
						i +
						(v.constraints.some((c) => c._constraintType === "prop-constraint")
							? "ἱ"
							: "");
					i = i + " ";
					//bindings
					...
					// backbone properties
					if (v.subdefs) {
						const desc: string = alternatingColor(
							`${k}${(i + s).padStart(8, " ") + " "}${c}${v.type}(BACKBONE)${b}`,
						);
						const subs: string = this.propToString(
							Array.from(v.subdefs.values()),
							indent + "    ",
						);
						return `${desc}\n${subs}`;
					}
...
export class Context<T extends Decorateable> {
	constructor(
		public name: ResourceDef | DatatypeDef,
		public def: StructureDefinition<T>,
	) {}
}
