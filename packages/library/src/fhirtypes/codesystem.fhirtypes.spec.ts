...
describe("Code Systems", () => {
	...
	describe("by kind", () => {
		test("should find by resource", () => {
			expect(FhirTypes.filter((t) => t.kind === "resource").length).to.equal(
				162,
			);
		});
		test("should find by primitive", () => {
			expect(FhirTypes.filter((t) => t.kind === "primitive").length).to.equal(
				21,
			);
		});
		test("should find by datatype", () => {
			expect(FhirTypes.filter((t) => t.kind === "datatype").length).to.equal(
				47,
			);
		});
	});
