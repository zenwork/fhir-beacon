 ...
 */
export function when<WHEN, THEN>(
	target: WHEN,
	...evaluations: Array<Evaluation<WHEN, THEN>>
): THEN {
	let then: THEN | null = null;

	// eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
	function extracted(getThen: (() => THEN) | THEN | (THEN & Function)) {
		then =
			typeof getThen === "function"
				? (getThen as Do<THEN>)()
				: (getThen as THEN);
		return true;
	}
	...
	evaluations.some((statement) => {
		...
		if (typeof condition === "function") {
			if ((condition as WhenEvaluation<WHEN>)(target))
				return extracted(statement[1]);
		} else {
...
export const otherwise: <WHEN, THEN>(
	t: (() => THEN) | THEN,
) => Evaluation<WHEN, THEN> = (t) => [() => true, t];
