...
import {elementLineageCtx, Lineage}             from '../contexts'
...
export abstract class FhirPresentableElement<T extends FhirElementData> extends FhirDataElement<T> {
  ...
  protected constructor(type: string) {
    ...
    this.#lineage = { labels: [this.label], types: [this.type] }

    new ContextConsumer(this, {
      context: elementLineageCtx,
      callback: (value: Lineage) => {
        this.#lineage = {
          labels: [...value.labels, this.label],
          types: [...value.types, this.type]
        }
      },
      subscribe: true
    })
